...
def scrape_main_page(url, page):
    ...
    for item in raws:
        ...
        raw = soup.find('a', href=event_url)

        if raw:
            # Find the direct child <div> with the class name "desc"
            desc_div = raw.find('div', class_='desc')
            event_title = desc_div.find('h3').text.strip()

            # Extract the date range and split into start and end dates
            date_range = desc_div.find('h2').find('span').text.strip()
            ...
            event_description = desc_div.find('p', class_='event-desc').text.strip()
